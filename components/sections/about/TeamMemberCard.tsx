    ...
    <>
          ...
          <div className="border-border/30 group-hover:border-accent/30 relative h-12 w-12 shrink-0 overflow-hidden rounded-full border transition-all duration-500">
            {
              ...
              /> : <span
                className="flex  font-semibold text-lg justify-center size-full items-center font-playfair"
              >
                {member.initials}
              </span>
            }
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="h-[80vh] max-h-150 max-w-md gap-0 overflow-hidden rounded-2xl border-0 p-0 shadow-2xl">
          ...
          {/* Image OR Initials fills entire modal */}
          {
            member.image ? (
              <Image
                fill
              ...
              <span
                className="flex justify-center items-center size-full font-playfair font-bold text-[clamp(10rem,20vw,1rem)]"
              >
                {member.initials}
              </span>
            )
          ...
          <div className="absolute right-0 bottom-0 text-center left-0 p-8">
            <h3 className="text-foreground font-playfair mb-1 text-4xl font-semibold">
              {member.name}
            </h3>
            <p className="text-foreground/80 mb-4 text-sm font-medium tracking-[0.1em] uppercase">
              {member.role}
            </p>
            ...
            <div className=" flex justify-center items-center gap-3.5">
              {member.linkedin && (
                <a
                  ...
                  className="border-foreground/20 hover:border-foreground/50 hover:bg-foreground/10 flex size-8 items-center justify-center rounded-full border transition-all duration-300"
                >
                  <Linkedin className="text-foreground/70 hover:text-foreground size-4 transition-colors" />
                </a>
              )}
              {member.twitter && (
                <a
                ...
                >
                  <Twitter
                    className="text-foreground/70 hover:text-foreground size-4 transition-colors"
                  />
                </a>
              )}
              {member.email && (
